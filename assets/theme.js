(function (factory) {
  ...
  function _isStorageSupported(storage) {
    try {
      const key = "__storage_availability";
        ...
        fastdom.mutate(function () {
          ...
          if (_this.options['filterType'] === 'group') {
            _this.element.querySelectorAll('.collection__filter-item-active').forEach(function (item) {
              ...
              var activeDataTag = parentList.querySelectorAll('.is-selected[data-tag]');

              if (activeDataTag.length > 0) {
                item.style.display = 'block';

                var activeTags = [];
                activeDataTag.forEach(function (tag) {
                  activeTags.push(tag.getAttribute('data-tag-user'));
                });

                if (activeTags.length == 1) {
                  item.innerText = activeTags[0];
                } else {
                  item.innerText = activeTags.length + ' ' + item.dataset.tSelected;
                  // List number of selected tags: activeTags.length + ' ' + item.dataset.tSelected;
                  // List all tags separated by comma: activeTags.join(', ');
                }
              } else {
      ...
      value: function _updateInventory(newVariant) {
        ...
        if (newVariant['available']) {
          if (null !== variantInventoryManagement && variantInventoryPolicy === 'deny' && this.options['lowInventoryThreshold'] > 0) {
            // Note (Vig) : this 2x multiplier is applied because there is a tier 2 scarcity message
            //              this logic needs to be kept in sync with logic in product-info.liquid
            if (variantInventoryQuantity <= this.options['lowInventoryThreshold'] * 2) {
  ...
  */
  ...
  var ProductSection = /*#__PURE__*/function () {
    ...
    _createClass(ProductSection, [{
      ...
      key: "_productNavigation",
      value: function _productNavigation() {
        /**
         * Check for PDP & localStorage availability
         */
        const productNav = document.querySelector('[data-section-id="product-template"] .page__sub-header');
        if (productNav == undefined || !_isStorageSupported(window.localStorage)) return;

        // Get localStorage data
        const navigationDataForProductPage = window.localStorage.getItem('_navigationDataForProductPage'),
          navData = JSON.parse(navigationDataForProductPage);

        // Vig : navData can be null in private browser modes when visiting a product without going through a collection
        if (navData == null) {
          return;
        }

        // Check if navigation expired
        if (navData.isExpired) return;

        // Navigation element nodes
        const collectionBreadcrumb = productNav.querySelectorAll('.breadcrumb__list .breadcrumb__item')[1],
          itemNavigation = productNav.querySelector('.page__navigation'),
          prevProduct = itemNavigation.querySelector('.page__navigation-item--prev a'),
          nextProduct = itemNavigation.querySelector('.page__navigation-item--next a');

        // Load data from localStorage
        const navigationData = navData.collectionNavigationDataForBreadcrumb,
          collectionData = navData.productsInCollectionForPrevNextFeature

        // Check for data in localStorage and hide prev/next in case of links pointing the same URL. This preserves their functionality when using collection name in URL
        if (navigationData == null || collectionData == null) {
          if (prevProduct.href == window.location.href) prevProduct.parentNode.remove();
          if (nextProduct.href == window.location.href) nextProduct.parentNode.remove();
          return;
        }

        // Check if collection in localStorage equals collection customer came from. If customer comes from link preserve default behavior
        if (navigationData.collectionHandle != collectionData.collection) return;

        // Actual product index in collection
        const actualIndex = collectionData.products.indexOf(String(window.location.pathname));

        // Prev/next buttons & breadcrumb logic
        if (navigationData.product == window.location.pathname) {
          if (navigationData.collectionHandle && navigationData.collectionTitle) {
            collectionBreadcrumb.querySelector('a').href = `/collections/${navigationData.collectionHandle}`;
            collectionBreadcrumb.querySelector('a').innerHTML = navigationData.collectionTitle;
          }
          if (actualIndex > 0) {
            prevProduct.href = collectionData.products[actualIndex - 1];
            prevProduct.addEventListener('click', () => {
              let newNavigationData = navigationData;
              newNavigationData.product = collectionData.products[actualIndex - 1];

              let newNavData = window.localStorage.getItem('_navigationDataForProductPage');
              newNavData = JSON.parse(newNavData);
              newNavData.collectionNavigationDataForBreadcrumb = newNavigationData;
              newNavData.isExpired = false;

              window.localStorage.setItem('_navigationDataForProductPage', JSON.stringify(newNavData));
            });
          } else {
            prevProduct.parentNode.remove();
          }
          if (actualIndex < collectionData.products.length - 1) {
            nextProduct.href = collectionData.products[actualIndex + 1];
            nextProduct.addEventListener('click', event => {
              let newNavigationData = navigationData;
              newNavigationData.product = collectionData.products[actualIndex + 1];
            ...
            nextProduct.parentNode.remove();
          }

          itemNavigation.classList.remove('hidden');

          let newNavData = window.localStorage.getItem('_navigationDataForProductPage');
          newNavData = JSON.parse(newNavData);
          newNavData.isExpired = true;
  ...
  var CollectionSection = /*#__PURE__*/function () {
    ...
    _createClass(CollectionSection, [{
      ...
      value: function _attachListeners() {
        ...
        this.delegateElement.on('click', '[data-action="add-to-cart"]', this._addToCart.bind(this));
        this.delegateElement.on('click', '.product-item__link', this._viewProduct.bind(this));
        ...
        if (body && body.dataset.lsUrls == 'true') {
          collections.forEach(collection => {
            collection.addEventListener('click', e => {
              let product_link = null;
    
              if (!e.target.closest('.boost-pfs-filter-products')) return;
    
              if (e.target.classList.contains('product-item__title') || e.target.classList.contains('product-item__image-wrapper')) {
                product_link = e.target;
              } else if (e.target.closest('a') && e.target.closest('a').classList.contains('product-item__image-wrapper')) {
                product_link = e.target.closest('a');
              } 
              ...
              if (product_link != null) {
                ...
                const new_link = product_link.href.split('/products/');
                product_link.href = (new_link.length > 0) ? `/products/${new_link[1]}` : `/products/${new_link[0]}`;
                _this._viewProduct('', product_link);
    
                window.location = product_link.href;
              }
            });
          });
        }
      ...
      value: function _tagToggled(event, target) {
        ...
        if (this.currentTags.includes(newTag)) {
          ...
          if (this.options['filterType'] === 'tag') {
            ...
            if (target.dataset.multipleFilters != 'true') {
      ...
      /**
       * ---------------------------------------------------------------------------------------------------
       * SAVE COLLECTION INFORMATION TO LOCALSTORAGE ON ENTER
       * ---------------------------------------------------------------------------------------------------
       */

    }, {
      key: "_viewProduct",
      value: function _viewProduct(event, target) {
        if (!_isStorageSupported(window.localStorage)) return;

        if (window.collectionsData) {
          const collectionEl = document.querySelector('.collection'),
            collectionData = (window.collectionsData.length > 1) ? window.collectionsData.filter(item => item.collection == collectionEl.dataset.handle) : window.collectionsData[0],
            navigationData = `{
          ...
          localStorage.setItem('_navigationDataForProductPage', navigationDataForProductPage);
        } else {
          localStorage.removeItem('_navigationDataForProductPage');
        }
      }
    }]);
      ...
      value: function _viewProduct(event, target) {
        ...
        if (window.collectionsData) {
          const collectionEl = target.closest('[data-section-type="featured-collection"]'),
            collectionData = window.collectionsData.length > 1 ? window.collectionsData.filter(item => item.collection == collectionEl.dataset.handle)[0] : window.collectionsData[0],
            navigationData = `{
              "product": "${target.pathname.replace('/product/','')}",
              "collectionTitle": "${collectionEl.dataset.title}",
              "collectionHandle": "${collectionEl.dataset.handle}"
  ...
  var JournalRecommended = /*#__PURE__*/function () {
    function JournalRecommended(element) {
      _classCallCheck(this, JournalRecommended);

      this.element = element;
      this.button = {
        prev: element.querySelector('.journal-recommended__button--prev'),
        next: element.querySelector('.journal-recommended__button--next')
      };
      this.flickityInstance = new flickityFade(element.querySelector('.popular-articles'), {
        pageDots: true,
        prevNextButtons: false,
        wrapAround: false,
        groupCells: true,
      ...
      if (this.button.prev && this.button.next) {
        this.button.next.addEventListener('click', () => this.flickityInstance.next());
        this.button.prev.addEventListener('click', () => this.flickityInstance.previous());

        if (!this.flickityInstance.isDraggable) {
          this.button.next.classList.add('disabled');
          this.button.prev.classList.add('disabled');
        }
        this.flickityInstance.on('change', index => {
          (index == 0) ? this.button.prev.classList.add('disabled') : this.button.prev.classList.remove('disabled');
          (index == this.flickityInstance.slides.length - 1) ? this.button.next.classList.add('disabled') : this.button.next.classList.remove('disabled');
        });
      }
    }

    _createClass(JournalRecommended, [{
      key: "onUnload",
      value: function onUnload() {
        this.flickityInstance.destroy();
      }
    ...
    return JournalRecommended;
  }();

  var JournalFeaturedSlider = /*#__PURE__*/function () {
    function JournalFeaturedSlider(element) {
      _classCallCheck(this, JournalFeaturedSlider);

      this.element = element;
      this.flickityInstance = new flickityFade(element, {
        pageDots: true,
        prevNextButtons: true,
        wrapAround: false,
        groupCells: true,
    ...
    _createClass(JournalFeaturedSlider, [{
      key: "onUnload",
      value: function onUnload() {
        this.flickityInstance.destroy();
      }
    ...
    return JournalFeaturedSlider;
  }();

  var JournalGallery = /*#__PURE__*/function () {
    function JournalGallery(element) {
      _classCallCheck(this, JournalGallery);

      this.element = element;
      this.mainButton = {
        prev: element.querySelector(':scope > .journal-gallery__controls .journal-gallery__controls-button--prev'),
        next: element.querySelector(':scope > .journal-gallery__controls .journal-gallery__controls-button--next')
      };
      this.popup = {
        el: element.querySelector('.journal-gallery__popup'),
        prev: element.querySelector('.journal-gallery__popup .journal-gallery__controls-button--prev'),
        next: element.querySelector('.journal-gallery__popup .journal-gallery__controls-button--next'),
        exit: element.querySelector('.journal-gallery__popup-exit')
      }
      this.flickityInstance = new flickityFade(this.element.querySelector('.journal-gallery__carousel'), {
        pageDots: false,
        prevNextButtons: false,
        wrapAround: false,
        groupCells: true,
      ...
      if (this.popup.el) {
        this.flickityInstancePopup = new flickityFade(this.element.querySelector('.journal-gallery__popup-carousel'), {
          pageDots: false,
          prevNextButtons: false,
          wrapAround: false,
          groupCells: true,
        ...
        this.popup.exit.addEventListener('click', () => {
          this.popup.el.setAttribute('aria-hidden', true);
          this.popup.el.classList.remove('visible');
        });

        this.popup.next.addEventListener('click', () => this.flickityInstancePopup.next());
        this.popup.prev.addEventListener('click', () => this.flickityInstancePopup.previous());

        this.flickityInstancePopup.on('change', index => {
          (index == 0) ? this.popup.prev.classList.add('disabled') : this.popup.prev.classList.remove('disabled');
          (index == this.flickityInstancePopup.slides.length - 1) ? this.popup.next.classList.add('disabled') : this.popup.next.classList.remove('disabled');
        });
        this.flickityInstance.on('staticClick', (event, pointer, cellElement, cellIndex) => {
          this.flickityInstancePopup.select(cellIndex, false, true);
          this.popup.el.setAttribute('aria-hidden', false);
          this.popup.el.classList.add('visible');
          this.flickityInstancePopup.resize();
        });
      }

      if (this.mainButton.prev && this.mainButton.next) {
        this.mainButton.next.addEventListener('click', () => this.flickityInstance.next());
        this.mainButton.prev.addEventListener('click', () => this.flickityInstance.previous());

        this.flickityInstance.on('change', index => {
          (index == 0) ? this.mainButton.prev.classList.add('disabled') : this.mainButton.prev.classList.remove('disabled');
          (index == this.flickityInstance.slides.length - 1) ? this.mainButton.next.classList.add('disabled') : this.mainButton.next.classList.remove('disabled');
        });
      }
    }

    _createClass(JournalGallery, [{
      key: "onUnload",
      value: function onUnload() {
        this.flickityInstance.destroy();
        this.popup.el && this.flickityInstancePopup.destroy();
      }
    }, {
      key: "onBlockSelect",
      value: function onBlockSelect(event) {
        if (this.flickityInstance.isActive) {
          ...
          if (this.popup.el) {
            this.flickityInstancePopup.selectCell(parseInt(event.target.getAttribute('data-block-index')), false, event.detail.load);
            this.flickityInstancePopup.pausePlayer();
          }
        }
      }
    }, {
      ...
      value: function onBlockDeselect() {
        ...
        if (this.popup.el && this.flickityInstancePopup.isActive) {
          this.flickityInstancePopup.unpausePlayer();
        }
      }
    }]);
  ...
  (function () {
    var bootTheme = function bootTheme() {
      ...
      sections.register('journal-recommended', JournalRecommended);
      sections.register('journal-featured-slider', JournalFeaturedSlider);
      sections.register('journal-gallery', JournalGallery);
      /**
       * ----------------------------------------------------------------------------
       * RTE
       * ----------------------------------------------------------------------------
